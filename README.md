...
시험 빌드다. 0.1.4보다 새롭고 정식 0.1.5보다 앞선다. 윈도우 실기에서 글씨
크기를 확인받기 위한 배포이며, 확인되면 그대로 0.1.5로 낸다.

- **UI 글씨 크기 상향**: 스타일시트의 글씨 크기가 대부분 8~10px이라 특히
  윈도우에서 읽기 어렵던 문제를 고쳤다. 맥 Retina는 기본 스케일링에서 CSS
  픽셀당 물리 픽셀이 많아 그럭저럭 읽혔지만, 윈도우는 100% 배율에서 둘이
  같아 같은 화면이 훨씬 작게 보인다. 가장 작은 글씨가 8px에서 12.2px가
  됐고, 이제 모든 크기가 하나의 값(`--ui-font-scale`)으로 함께 조절된다.

이전 버전 (v0.1.4)

- **마이크 캘리브레이션 부호 수정 (중요)**: 캘리브레이션 TXT가 기록한 마이크
  편차를 나눠 내야 하는데 곱해 넣고 있어, 마이크 오차가 두 배로 측정에 남고 그대로
  필터에 들어가던 문제를 고쳤다. 90도 UMIK 파일 기준 8~20 kHz에 존재하지 않는
  절벽이 만들어지던 결함이다. 기존 측정 캐시는 보존된 원본 WAV를 현재
  파이프라인으로 다시 분석해 복원한다(재측정 불필요).
- **기본 스윕 앱 내장**: L/R 스윕 WAV를 앱에 내장해 세션을 시작하면 자동으로
  올라간다. 더 이상 스윕 파일을 따로 받아 올릴 필요가 없다(다른 파일로 교체·복구
  가능).
- **기본 타깃 곡선 = Harman-6dB (출처: Dirac)**: B&K 스타일·Harman 스타일 선택지를
  없애고 Harman-6dB를 기본값으로 탑재했다. **고역 롤오프는 측정치를 따라간다** —
  고역이 잘 뻗는 스피커는 −1 dB/oct 기울기를 끝까지 쓰고, 위쪽이 롤오프하는
  스피커는 그 감쇠를 따라 꺾여 낼 수 없는 부스트를 요구하지 않는다.
- **최종 Roon ZIP 실패 수정**: 44.1 kHz 계열 멤버의 20~100 Hz 군지연이 48 kHz
  멤버와 33.6 ms 어긋나 패키지 생성이 실패하던 문제를 고쳤다. 이제 48 kHz 멤버
  하나만 설계하고 나머지 레이트는 그것을 리샘플한다.
- **캐시 불러오기 속도 개선**: 복원 작업을 메인 스레드 밖으로 옮기고 중복을 먼저
  걸러내며 재계산 결과를 사이드카로 남긴다(약 59초 정지 → 응답 유지, 두 번째부터
  1~2초).

이전 버전 (v0.1.3)

- **개량 SECS 기본 적용**: 실현 불가능한 위상 보정을 그대로 두는 가드와, 설계된
  필터의 군지연을 직접 재는 게이트를 넣었다. 이전 필터가 저역을 70~200 ms 늦추던
  문제를 잡는다.
- **지연 상한 자동 결정**: 방의 저음이 실제로 얼마나 늦게 도착하는지 측정해 필요한
  만큼만 지연 예산을 쓴다(늦은 저음이 없으면 원본 10 ms 유지). 실측 예: 필요 56 ms →
  상한 70 ms 자동 선택, 청취점 40~70 Hz 도달 오차 +55.6 → +1.6 ms.
- **서브우퍼 결합 검색 v5**: 후보 크로스오버 목록을 바꾸면 목록에 계속 있던 후보의
  최적값까지 달라지던 문제를 고쳤다(도달 시각 추정·채점 대역·탐색 격자가 후보 목록에
  의존하던 세 지점 제거).
- **권장 헤드룸 정정**: 프로그램 피크 측정이 고정 주파수 격자를 쓰다 70 Hz 부근
  베이스에서 1.4 dB 과소 권장하던 문제를 주파수 스캔으로 교체해 고쳤다.
- **9지점 측정 배치 변경**: P0를 정중앙으로 두고 P1~P8을 직육면체 여덟 꼭짓점으로
  배치한다(위층 4곳 → 아래층 4곳 순서라 스탠드 높이를 한 번만 바꾼다).
...
캘리브레이션 파일은 **선택 사항**입니다. 보정이 마이크 자체에 내장된 측정 마이크라면
파일을 올리지 마세요 — 올리면 보정이 두 번 적용됩니다. 파일 없이 진행하면 경고가
표시되고, 내보내는 패키지에도 무보정(uncalibrated)으로 기록됩니다.

L/R 스윕은 앱에 내장된 기본 스윕이 자동으로 올라가 있으므로 그냥 두시면 됩니다.
다른 스윕으로 측정해야 할 때만 **다른 스윕 WAV 선택**으로 바꾸고, **기본 스윕으로
되돌리기**로 언제든 되돌릴 수 있습니다.

스윕을 틀 별도의 플레이어가 없다면 이 화면의 **출력 장치 (선택)**에서 스피커를
고르세요. 그러면 서브우퍼 단계와 다지점 측정 단계에 "앱에서 스윕 재생" 체크박스가
켜지고, 측정 시작 버튼을 누른 뒤 3초 있다가 앱이 직접 스윕을 재생합니다. 스피커를
고르지 않으면 앱은 출력 장치를 검색하지도 열지도 않습니다. 어느 쪽이든 스윕을
인식하고 판정하는 방식은 완전히 같습니다.
