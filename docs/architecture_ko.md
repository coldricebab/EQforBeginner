...
| 고급 무선 측정 전송 | 사용자가 Roon에서 직접 시작하는 로컬 WAV 재생; 마이크 측 인식/디컨볼루션 | `wireless-sweep-recognition-v1` + `known-sweep-deconvolution-v6` | 안전하지 않은 원격 전송/음량 변경을 피한다. 임의의 네트워크 지연은 탐색되는 캡처 오프셋이 되고, 고정된 부호 있는 사전 제로 윈도가 음향 마커 기준보다 먼저 도달하는 경로를 보존한다 |
...
- `measurement` (부분 구현): 가져온 48 kHz WAV를 디코딩하고 SHA-256으로
  ...
  독립 세그먼트 검사로 실제 모노 캡처를 탐색한다. `known-sweep-deconvolution-v6`는
  ...
  스윕 쌍은 `include_bytes!`로 데스크톱 실행 파일에 내장되어 있고 세션이 시작될 때
  좌·우 두 채널로 import되므로, 파일을 고르지 않은 사용자도 검증된 스윕으로 측정할
  수 있고 패키징된 앱은 저장소를 옆에 둘 필요가 없다. 이 파일은 픽스처가 쓰는
  `assets/sweeps/*_refR.wav`와 바이트 단위로 동일하며 테스트가 이를 고정한다. 수용
  측정 캐시가 스윕의 SHA-256을 키로 쓰므로, 비슷하기만 한 기본값은 그 이전에 측정한
  모든 세션을 무효화하기 때문이다. 내장 바이트는 IPC 경계를 넘지 않으며, 기본값과
  사용자 선택 파일은 동일한 import 경로를 공유하므로 어떤 검증·증거 사본·후단
  단계도 둘을 구분하지 못한다.
...
출력 구성을 검사하거나, 출력 스트림을 열지 않는다. 출력 장치 열거는 별도 명령
`scan_default_host_outputs`가 담당하며, 사용자가 앱 내 스윕 재생을 위해 스피커를
요청했을 때만 실행된다. 스윕을 다른 곳에서 재생하는 세션에서는 출력 장치를
조회하지도 열지도 않는다. 입력
...
고급 무선 경로와 완전한 라이브 경로는 스스로 어떤 오디오도 출력하지 않는다.
...
EQforBeginner는 물리적 앰프 레벨을 알 수 없고, 사용자가 요청하지 않은 스윕 재생은
피할 수 있는 안전 위험을 만들기 때문이다.

유일한 예외는 두 번에 걸쳐 명시적으로 옵트인된 경우다. 사용자가 스피커를 고르고
재생 체크박스를 켜 둔 경우에만, 캡처가 시작된 뒤 고정된 3초의 여유를 두고 가져온
스윕을 그 장치에서 재생한다. 앰프 레벨은 여전히 사용자의 몫이며, 이 기능은 하류에
아무 영향도 주지 않는다. 재생은 자체 취소 상태를 가진 별도 명령이고, 판정에
아무것도 보고하지 않으며, 재생이 실패해도 캡처는 실패하거나 달라지지 않는다.
판정은 스윕이 이 명령에서 나왔든 Roon에서 나왔든 동일한 코드 경로로, 실제로
마이크에 도달한 신호만 분석한다. 3초는 청취자가 자리를 잡기 위한 편의일 뿐 동기화
신호가 아니다. 타임라인은 아래의 마커 탐색이 확립한다.
